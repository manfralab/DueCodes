...
def list_measured_params(dat, print_=False):
    ds = load_by_id(dat)
...
def get_data_from_ds(dat, param_name, dtype='numpy'):
    ''' return parameter data from dataset
        if dtype=='numpy' returns a dict of numpy arrays
        if dtype=='pandas' returns a multi indexed dataframe '''
