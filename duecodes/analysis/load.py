...
def get_data_as_xarray(identifier: Union[int, str], dependent_params: List[str] = None):

    df = get_data_as_dataframe(identifier, dependent_params=dependent_params)
    return dataframe_to_xarray(df)


def get_data_as_dataframe(
    identifier: Union[int, str], dependent_params: List[str] = None
):
    ...
    setpoints = tuple(np.array(vals).flatten() for vals in setpoint_dict.values())
    multi_index = pd.MultiIndex.from_tuples(zip(*setpoints), names=setpoint_dict.keys())
    data = {}
